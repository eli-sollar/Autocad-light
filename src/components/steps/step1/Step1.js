  ...
  const handleSubmit = (event) => {
    if (store.planner || store.customers || store.roofTypeId) {
      ...
      if (store.roofTypeId) {
        ...
        parameters["roofTypeText"] = store.roofTypes.find(
          (x) => x.Key == store.roofTypeId
        ).Value;
      }
      if (projectId) {
  ...
  return (
    <form onSubmit={handleSubmit} id={`step${activeId}`}>
      ...
      <div className="d-flex justify-content-around select-3">
        ...
        {store.dataLoaded && (
          <SelectCustom
            onChange={(data, type) => actions.setCustomers(dispatch, data)}
            defaultData={store.customersDefault}
            name="Customers"
      ...
      <div className="mt-5">
        ...
        <RadioGroup
          ...
          onChange={(e) =>
            actions.setRoofSelected(dispatch, e.currentTarget.value)
          }
        >
          {Object.keys(store.roofTypes).map(
            (key) =>
              store.roofTypes[key].ImageName && (
                <FormControlLabel
                  labelPlacement="bottom"
                  key={store.roofTypes[key].Key}
                  value={store.roofTypes[key].Key}
                  label={store.roofTypes[key].Value}
                  control={
                    <Radio
                      icon={
                        <ImageWrapper src={store.roofTypes[key].ImageName} />
                      }
                      checkedIcon={
                        <ImageWrapper
                          class="img-selected"
                          src={store.roofTypes[key].ImageName}
                        />
                      }
                    ></Radio>
                  }
      ...
      {store.roofTypeId != constants.ROOF_TYPE.FLAT && (
        <Container className="pe-4">
          ...
          <RadioGroup
            ...
            onChange={(e) =>
              actions.setFetotDirection(dispatch, e.currentTarget.value)
            }
            id="fetot-direction"
          >
